<template>
  <div class="content-top-button">
    ...
    <el-tooltip class="item" effect="dark" content="预览" placement="top-start">
      <el-button type="text" icon="el-icon-view" size="mini" @click="viewFrom"
        >预览</el-button
      >
    </el-tooltip>
    <el-dialog
      v-if="formViewVisible"
      title="预览"
      :visible.sync="formViewVisible"
      :top="'10vh'"
      append-to-body
    >
      <TFormBuild :options="data" :dynamic-data="dynamicData" />
    </el-dialog>
    <!--    导入Json-->
    ...
    <el-dialog
      v-if="importJsonView"
      title="导入Json(请注意数据格式)"
      :visible.sync="importJsonView"
      :top="'10vh'"
      append-to-body
    >
      <ImportJson :import-json="importJson" />
      <span slot="footer" class="dialog-footer">
        <el-button @click="importJsonView = false">取 消</el-button>
        <el-button type="primary" @click="handleEmitImportJson"
          >确 定</el-button
        >
      </span>
    </el-dialog>
    ...
    <el-dialog
      v-if="jsonViewVisible"
      title="Json数据如下所示"
      :visible.sync="jsonViewVisible"
      :top="'10vh'"
      append-to-body
    >
      <CodemirrorVue :json="JSON.stringify(data, null, '\t')" />
    </el-dialog>
