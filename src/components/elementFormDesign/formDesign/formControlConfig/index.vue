<template>
  <div>
    <el-form
      ...
      size="mini"
    >
      <p style="text-align: center; margin: 6px 0; font-size: 12px">
        {{ config.key }}
      </p>
      <el-button
        type="primary"
      ...
      <el-divider v-if="config.options.hasOwnProperty('labelWidth')" />
      <el-form-item v-if="config.options.hasOwnProperty('labelWidth')" label="自定义label宽度（如：50px）">
        <el-input v-model="config.options.labelWidth" clearable />
      </el-form-item>
      <el-divider v-if="config.options.hasOwnProperty('customizeName')" />
      <el-form-item v-if="config.type==='customize'" label="自定义组件名称（小驼峰命名）">
        <el-input v-model="config.options.customizeName" clearable />
      </el-form-item>
      <el-divider v-if="config.options.hasOwnProperty('buttonType')" />
      <el-form-item
        v-if="config.options.hasOwnProperty('buttonType')"
        label="按钮类型"
      >
        <el-radio-group v-model="config.options.buttonType" size="mini">
          <el-radio-button label="primary" type="primary"
            >primary</el-radio-button
          >
          <el-radio-button label="success" type="success"
      ...
      <el-form-item
        v-if="config.options.hasOwnProperty('handle')"
        label="按钮类型"
      >
      ...
      <el-form-item
        v-if="config.options.hasOwnProperty('url')"
        label="连接地址"
      >
        <el-input v-model="config.options.url" clearable />
      </el-form-item>
      <el-form-item
        v-if="
          config.options.hasOwnProperty('handle') &&
            config.options.handle === 'doSomething'
        "
        label="自定义事件"
      >
        <el-input
      ...
      <el-form-item v-if="config.hasOwnProperty('model')" label="数据字段">
        <el-input v-model="config.model" clearable />
      </el-form-item>

      <el-divider v-if="config.options.hasOwnProperty('width')" />
      <el-form-item
        v-if="config.options.hasOwnProperty('width')"
        :label="
          config.type === 'button' ? '按钮位置' : '宽度(子表中控件宽度不生效)'
        "
      >
        <div class="block">
          <el-slider
      ...
      <el-divider v-if="config.options.hasOwnProperty('dTableDialog')" />
      <div v-if="config.options.hasOwnProperty('dTableDialog')">
        <!-- <span>1、弹窗基本属性---参数配置</span> -->
        <el-form-item label="1、弹窗标题">
          <el-input v-model="config.options.dTableDialog.title" clearable />
        </el-form-item>
        <el-form-item label="2、弹窗宽度">
          <el-input
            v-model="config.options.dTableDialog.width"
            placeholder="请输入数字"
            clearable
          />
        </el-form-item>
        <!-- <span>2、弹窗接口---参数配置</span> -->
        <el-form-item label="3、接口url">
          <el-input
            v-model="config.options.dTableDialog.request.url"
            clearable
          />
        </el-form-item>
        <el-form-item label="4、接口请求类型">
          <el-select
            v-model="config.options.dTableDialog.request.method"
            placeholder="请选择"
          >
        ...
        <span class="ebank-label">5、弹窗表头---参数配置</span>
        <OptionAdd
          v-model="config.options.dTableDialog.tableHeaders"
          :type="config.type"
        />

        <!-- <span>4、弹窗选中属性---参数配置</span> -->
        <el-form-item label="6、表格选中属性">
          <el-input v-model="config.options.dTableDialog.propName" clearable />
        </el-form-item>
      </div>
      ...
      <el-form-item
        v-if="config.options.hasOwnProperty('gutter')"
        label="格栅间距"
      >
        <el-input-number
      ...
      <div
        v-if="config.hasOwnProperty('columns') && config.type === 'grid'"
        ...
        >
          ...
          <el-col :span="6">
            <el-button
              ...
              icon="el-icon-delete"
              circle
              @click="deleteItem(config.columns, index)"
            />
          </el-col>
        </el-row>
        <a
          ...
          @click="addItem(config.columns)"
          >增加数据</a
        >
      </div>
      <el-divider v-if="config.options.hasOwnProperty('options')" />
      <el-form-item
        v-if="config.options.hasOwnProperty('options')"
        label="下拉框数据"
      >
        ...
        <OptionAdd
          v-if="!config.options.dynamic"
          v-model="config.options.options"
          :type="config.type"
        />
        ...
        <div style="display:inline">
          <el-input
            v-if="config.options.dynamic"
            v-model="config.options.dynamicKey"
            style="margin-top: 10px;width:70%;"
            placeholder="请输入动态数据地址"
          />
          <el-button
            v-if="config.options.dynamic"
            type="danger"
            style="padding-left: 2.5px; padding-right: 2.5px;width:20%;"
            @click="showDict"
            >选择</el-button
          >
        </div>
      ...
      <el-divider
        v-if="
          config.options.hasOwnProperty('showAllLeavels') &&
      ...
      <el-form-item
        ...
        label="显示选中值的完整路径"
      >
        <el-switch v-model="config.options.showAllLeavels" />
      </el-form-item>
      ...
      <div v-if="config.type === 'tabs'">
        <el-form-item v-if="config.type === 'tabs'" label="页签配置">
          <el-row v-if="config.type === 'tabs'" :gutter="8">
            <div
              v-for="(val, index) in config.columns"
              :key="index"
            ...
            >
              ...
              /></el-col>
              <el-col :span="9"
                ><el-input v-model="val.value" placeholder="值"
              /></el-col>
              <el-col :span="6">
                <el-button
                  style="padding: 7px"
                  ...
                  icon="el-icon-delete"
                  circle
                  @click="deleteTabsItem(config.columns, index)"
                />
              </el-col>
            </div>
            <el-col :span="24"
              ><el-button
                icon="el-icon-plus"
                @click="addTabsItem(config.columns)"
                >添加</el-button
              ></el-col
            >
          </el-row>
      ...
      <div v-if="config.type === 'progress'">
        <el-form-item v-if="config.type === 'progress'" label="默认值">
          <el-input-number
            v-model="config.options.defaultPercentage"
            :min="0"
            :max="100"
          />
        </el-form-item>
          ...
          <el-row v-if="config.type === 'progress'">
            <div v-for="(val, index) in config.options.colors" :key="index">
              <el-row>
                ...
                <el-col :span="12"
                  ><el-input v-model="val.percentage" placeholder="值"
                /></el-col>
                <el-col :span="6">
                  <el-button
                    type="danger"
                    icon="el-icon-delete"
                    circle
                    @click="deleteColorItem(config.options.colors, index)"
                  />
                </el-col>
              </el-row>
            </div>
            <el-col :span="24"
              ><el-button
                icon="el-icon-plus"
                @click="addColorItem(config.options.colors)"
                >添加</el-button
              ></el-col
            >
          </el-row>
        ...
        <el-form-item v-if="config.type === 'progress'" label="进度条宽度">
          <el-input-number
            v-model="config.options.strokeWidth"
            :min="0"
            :max="10000"
          />
        </el-form-item>
        <el-form-item
          v-if="config.type === 'progress'"
          label="环形进度条画布宽度"
        >
          <el-input-number
            v-model="config.options.circleWidth"
            :min="0"
            :max="10000"
          />
        </el-form-item>
      ...
      <div v-if="config.type === 'customAlert'">
        ...
        <el-form-item label="辅助描述">
          <el-input
            v-model="config.options.description"
            type="textarea"
            :rows="6"
          />
        </el-form-item>
      ...
      <div v-if="config.type === 'tree'">
        ...
        <el-form-item label="配置选项">
          <el-input
            v-model="config.options.defaultProps"
            type="textarea"
            :rows="6"
          />
        </el-form-item>
      ...
      <div
        v-if="
          config.options.hasOwnProperty('eventName') &&
            config.options.hasOwnProperty('eventNameOptions')
        "
      >
        <el-divider />
        ...
        <el-form-item label="触发事件类型（必须源控件支持！）">
          <el-select v-model="config.options.eventName">
            <el-option
      ...
      <el-form-item
        v-if="config.options.hasOwnProperty('chooseMax')"
        label="最多可选数"
      >
        <el-input-number
      ...
      <el-form-item
        v-if="config.options.hasOwnProperty('chooseMin')"
        label="最少可选数"
      >
        <el-input-number
      ...
      <el-divider
        v-if="
          config.options.hasOwnProperty('defaultValue') &&
            !config.options.hasOwnProperty('isChooseTimes')
        "
      />
      <el-form-item
        v-if="
          config.options.hasOwnProperty('defaultValue') &&
            !config.options.hasOwnProperty('isChooseTimes')
        "
        :label="config.type === 'number' ? '默认值(必须为数字)' : '默认值'"
      >
        <!-- <div>111这里修改为动态数据</div> -->
        <!-- 只有下拉选择框会有 -->
        <el-select
        ...
        >
          ...
          /> -->
          <el-option
            v-for="(items, index) in selectDynamicOptions"
            :key="index"
            :label="items.label"
            :value="items.value"
          />
        ...
        <el-cascader
          ...
          filterable
        />
        <el-input v-else v-model="config.options.defaultValue" clearable />
      </el-form-item>
      <el-divider v-if="config.options.hasOwnProperty('showInput')" />
      <el-form-item
        v-if="config.options.hasOwnProperty('showInput')"
        label="是否显示计数器"
      >
        <el-switch
      ...
      <el-form-item
        v-if="config.options.hasOwnProperty('switchValue')"
        label="默认值"
      >
        <el-switch
      ...
      <el-form-item
        v-if="config.options.hasOwnProperty('inactiveText')"
        label="左侧提示文字"
      >
      ...
      <el-form-item
        v-if="config.options.hasOwnProperty('activeText')"
        label="右侧提示文字"
      >
        <el-input v-model="config.options.activeText" />
      </el-form-item>
      <TimeOrDate
        v-if="config.options.hasOwnProperty('isChooseTimes')"
        v-model="config"
      />
      ...
      <el-form-item
        v-if="config.options.hasOwnProperty('numberDefaultValue')"
        label="默认值"
      >
        <el-input-number
      ...
      <el-form-item
        v-if="config.options.hasOwnProperty('precision')"
        label="精度(小数位数)"
      >
      ...
      <el-divider
        v-if="
          config.options.hasOwnProperty('placeholder') &&
            !config.options.hasOwnProperty('isChooseTimes')
        "
      />
      <el-form-item
      ...
      <el-form-item v-if="showCheckbox" label="操作">
        <el-checkbox
          v-if="config.options.hasOwnProperty('disabled')"
          v-model="config.options.disabled"
          >禁用</el-checkbox
        >
        <el-checkbox
          v-if="config.options.hasOwnProperty('hidden')"
          v-model="config.options.hidden"
          >隐藏</el-checkbox
        >
        <el-checkbox
          v-if="config.options.hasOwnProperty('clearable')"
          v-model="config.options.clearable"
          >可清除</el-checkbox
        >
        <el-checkbox
          v-if="config.options.hasOwnProperty('multiple')"
          v-model="config.options.multiple"
          >可多选</el-checkbox
        >
      </el-form-item>
      ...
      <el-form-item
        v-if="config.hasOwnProperty('rules')"
        label="校验规则(子表中校验规则不生效)"
      >
        <div
          v-for="(item, index) in config.rules"
          :key="index"
          style="margin-top: 6px"
        >
          <el-row v-if="index === 0">
            <el-col :span="24">
              <el-checkbox
                v-if="config.hasOwnProperty('rules')"
                v-model="config.rules[0].required"
                >必填项</el-checkbox
              >
            </el-col>
          </el-row>
          <el-row v-else>
            ...
            <el-col :span="18">
              <el-input v-model="item.message" placeholder="请输入提示内容" />
            </el-col>
            <el-col :span="6">
              <el-button
                type="danger"
                icon="el-icon-delete"
                circle
                @click="deleteRule(config.rules, index)"
              />
            </el-col>
          </el-row>
        </div>
...
export default {
  ...
  computed: {
    showCheckbox() {
      ...
      return (
        options.hasOwnProperty("clearable") ||
        options.hasOwnProperty("hidden") ||
        options.hasOwnProperty("disabled")
      );
    },
    selectDefaultValue: {
      get() {
        return this.config.options.multiple
          ? this.config.options.multipleDefaultValue
          : this.config.options.defaultValue;
      },
      set(val) {
        this.config.options.multiple
          ? (this.config.options.multipleDefaultValue = val)
          : (this.config.options.defaultValue = val);
      }
    }
  },
  watch: {
    // 下拉选择器 && 动态数据字典变化
    "config.options.dynamicKey": {
      handler(newValue, oldValue) {
        this.setSelectDynamicOptions();
      },
      deep: true,
      immediate: true
    },
    // 下拉选择器 && 静态动态数据切换
    "config.options.dynamic": {
      handler(newValue, oldValue) {
        this.setSelectDynamicOptions();
      },
      deep: true,
  ...
  methods: {
    // 下拉选择框&&切换动态表单时的下拉选择
    async setSelectDynamicOptions() {
      if (this.config.type !== "select") return;
      let dynamic = this.config.options.dynamic;
      let dictKey = this.config.options.dynamicKey;
      if (!dynamic) {
        this.selectDynamicOptions = this.config.options.options;
        this.selectDefaultValue = "";
      } else if (dynamic && dictKey) {
        this.selectDynamicOptions = await this.$store.dispatch(
          "form/fetchDictByKey",
          dictKey
        );
        this.selectDefaultValue = "";
      } else {
        this.selectDynamicOptions = [];
        this.selectDefaultValue = "";
      }
    },
    addItem(columns) {
