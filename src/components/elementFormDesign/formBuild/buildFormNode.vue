<template>
  <!-- 多种布局：1、表格布局；2、格栅布局；3、标签页布局；4、子表；5、卡片布局；6、普通布局 -->
  <div>
    <!-- 1、表格布局（开始）-->
              ...
              <FinalForm
                ...
                @onSuccess="
                  (response, file, fileList) => {
                    ...
                    $emit('onSuccess', response, file, fileList, model);
                  }
                "
                @beforeUpload="
                  file => {
                    const model = item.model;
                    $emit('beforeUpload', file, model);
                  }
                "
              />
            </td>
    ...
    <template v-if="item.type === 'grid'">
      <div class="form-build-grid">
        <el-row :gutter="item.options.gutter">
          <el-col
    ...
    <!-- 4、子表（开始）-->
    <template
      v-else-if="item.type === 'childTable' || item.type === 'childTablePage'"
    >
      <!-- <div>{{ formData[item.model] }}</div>
      <div>{{ item.options.pageIndex }}</div>
      <div>{{ item.options.pageSize }}</div>
      <div>{{ (item.options.pageIndex - 1) * item.options.pageSize }}</div>
      <div>{{ item.options.pageIndex * item.options.pageSize }}</div> -->
      <!-- :label-width="isLabel ? '0px' : (item.options.labelWidth ? item.options.labelWidth : layout.labelWidth + '%')" -->
       <!-- :label-width="isLabel ? '0px' : item.options.labelWidth" -->
      <el-form-item
        :prop="item.model"
        :label="item.options.hidden || isLabel ? '' : item.label"
        :required="item.rules ? item.rules[0].required : false"
        :label-width="formItemLableWidth"
      ...
      >
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <!-- 输入框(含搜索弹窗) （开始）-->
              <!-- <div v-if="column.type === 'inputSearch'">输入框(含搜索弹窗)---子表渲染</div> -->
              <!-- <div v-if="column.type === 'inputSearch'">11{{column.model}}</div> -->
              <el-input
                v-if="column.type === 'inputSearch'"
                suffix-icon="el-icon-search"
                v-model="
                  formData[item.model].slice(
                    (item.options.pageIndex - 1) * item.options.pageSize,
                    item.options.pageIndex * item.options.pageSize
                  )[scope.$index][column.model]
                "
                ...
                :style="
                  'width:' +
                    ...
                    '%;' +
                    column.options.customStyle
                "
                @[eventName]="dynamicFun(column)"
                @focus="inputSearchFocusHandler(column,scope.$index,i)"
              />
              <!-- 输入框(含搜索弹窗) （结束）-->
              <el-input
              ...
              <treeselect
                ...
                :default-value="
                  column.options.multiple
                    ? column.options.multipleDefaultValue
              ...
              <el-cascader
                ...
                "
                :options="
                  column.options.dynamic &&
                  dynamicData[column.options.dynamicKey]
                    ? dynamicData[column.options.dynamicKey]
              ...
              >
                <!-- 这里需要修改？？？ -->
                <!-- v-for="(items,index) in (item.options.dynamic ? selectionDynamicOptions:item.options.options)" -->
                <!-- <el-option
                  v-for="(items, index) in column.options.dynamic &&
                  dynamicData[column.options.dynamicKey]
                    ? dynamicData[column.options.dynamicKey]
                    : column.options.options"
                  :key="index"
                ...
                <el-option
                  v-for="(items, index) in column.options.dynamic &&
                  column.options.dynamicKey
                    ? childTableSelectDictObj[column.options.dynamicKey]
                    : column.options.options"
                  :key="index"
                  :label="items.label"
                  :value="items.value"
              ...
              <el-date-picker
                v-if="
                  column.type === 'date' && column.options.isChooseTimes === '2'
                "
                v-model="
                  formData[item.model].slice(
                    (item.options.pageIndex - 1) * item.options.pageSize,
                ...
                :disabled="disabled ? disabled : column.options.disabled"
                :placeholder="
                  column.options.isChooseTimes === '2'
                    ? ''
              ...
              <el-date-picker
                v-if="
                  column.type === 'date' && column.options.isChooseTimes === '1'
                "
                v-model="
                  formData[item.model].slice(
                    (item.options.pageIndex - 1) * item.options.pageSize,
              ...
              <el-date-picker
                v-if="
                  item.type === 'datetime' && item.options.isChooseTimes === '2'
                "
                v-model="
                  formData[item.model].slice(
                    (item.options.pageIndex - 1) * item.options.pageSize,
                ...
                :type="
                  item.options.isChooseTimes ? 'datetimerange' : 'datetime'
                "
              ...
              <el-date-picker
                v-if="
                  item.type === 'datetime' && item.options.isChooseTimes === '1'
                "
                v-model="
                  formData[item.model].slice(
                    (item.options.pageIndex - 1) * item.options.pageSize,
              ...
              <el-switch
                ...
                @[eventName]="dynamicFun(column)"
              />
              <div v-if="column.type === 'slider'" class="block">
                <el-slider
                  v-model="
                    formData[item.model].slice(
                      (item.options.pageIndex - 1) * item.options.pageSize,
          ...
          <el-table-column fixed="right" label="操作" width="100">
            <template slot="header" slot-scope="scope">
              <el-button
                ...
                circle
                class="table-button"
                @click="handleAdd(formData[item.model], item.model, item)"
              />
            </template>
            <template slot-scope="scope">
              <el-button
        ...
        <div v-if="item.type === 'childTablePage'" class="page-footer">
          <!-- <div v-if="item.type === 'childTable'" class="page-footer"> -->
          <pagination
            v-show="item.options.pageTotal > 0"
            :total="item.options.pageTotal"
            :page-sizes.sync="item.options.pageSizes"
            :page.sync="item.options.pageIndex"
            :limit.sync="item.options.pageSize"
            style="float:right;"
            @pagination="queryList(item, formData)"
          />
        </div>
      </el-form-item>
    </template>
    ...
    <template v-else>
      ...
      >
        <el-rate
          ...
          :style="
            'width:' + item.options.width + '%;' + item.options.customStyle
          "
        ...
        <treeselect
          ...
          :default-value="
            item.options.multiple
              ? item.options.multipleDefaultValue
        ...
        />
        <!-- 输入框(含搜索弹窗) （开始）-->
        <!-- <div v-if="item.type === 'inputSearch'">输入框(含搜索弹窗)---渲染</div> -->
        <el-input
        ...
        <el-cascader
          ...
          v-model="formData[item.model]"
          :options="
            item.options.dynamic && dynamicData[item.options.dynamicKey]
              ? dynamicData[item.options.dynamicKey]
        ...
        >
          ...
          /> -->
          <el-option
            v-for="(items, index) in item.options.dynamic
              ? selectionDynamicOptions
              : item.options.options"
            :key="index"
            :label="items.label"
            :value="items.value"
        ...
        <el-date-picker
          ...
          :disabled="disabled ? disabled : item.options.disabled"
          :placeholder="
            item.options.isChooseTimes === '2' ? '' : item.options.placeholder
          "
          @[eventName]="dynamicFun(item)"
        />
        <el-date-picker
        ...
        <el-switch
          ...
          @[eventName]="dynamicFun(item)"
        />
        <div v-if="item.type === 'slider'" class="block">
          <el-slider
            v-model="formData[item.model]"
            :default-value="item.options.numberDefaultValue"
            :disabled="disabled ? disabled : item.options.disabled"
        ...
        <el-button
          ...
          :disabled="disabled ? disabled : item.options.disabled"
          :style="
            'margin-left:' + item.options.width + '%' + item.options.customStyle
          "
    ...
    <!-- 动态表格弹窗（开始） -->
    <d-table-dialog
      v-if="(type === 'childTable' && dTableDialgoItemIndex.headerName) || type === 'inputSearch'"
      :inputDefaultValue="type === 'inputSearch' ? formData[item.model] : formData[item.model][dTableDialgoItemIndex.scopeIndex][dTableDialgoItemIndex.headerName]"
      ...
      @updateInputDefaultValue ="updateInputDefaultValue"
    ></d-table-dialog>

    <d-table-dialog
      v-if="type === 'childTable' || type === 'inputSearch'"
      :inputDefaultValue="type === 'inputSearch' ? formData[item.model] : inputDefaultName"
      :propName="dTableDialog.propName"
      :request="dTableDialog.request"
      :title="dTableDialog.title"
      :visible.sync="dTableDialog.visible"
...
export default {
  ...
  data() {
    return {
      // childTableSelectDictArr: [],
      selectionDynamicOptions: [], // 下拉框的动态数据
      childTableSelectDictObj: {}, // 子表&&所有下拉框动态数据
      dTableDialog:{},
      dTableDialgoItemIndex:{
        scopeIndex:0, // 第几行
        headerName:'', // 字段名
      },
      itemKey: Math.random(),
  ...
  created() {
    this.getSelectionDynamicOptions();
    this.getChildTableSelectDictObj();
  },
  computed: {
    // el-form-item的label-width宽度
    formItemLableWidth(){
      if(this.isLabel) return '0px';
      // 最内层的宽度
      if(this.item.options.labelWidth) return this.item.options.labelWidth;
      // 外层宽度控制
      if(this.layout.labelWidth){
        return this.layout.labelWidthUnitPercent ? this.layout.labelWidth+'%' : this.layout.labelWidth+'px';
      }
    },
    inputDefaultName(){
      if(this.type !== "childTable" || !this.dTableDialgoItemIndex.headerName || !this.dTableDialgoItemIndex.visible) return ''
      return this.formData[this.item.model][this.dTableDialgoItemIndex.scopeIndex][this.dTableDialgoItemIndex.headerName]
    },
    type() {
    ...
    childTableSelectDictArr() {
      let arr = [];
      if (this.type !== "childTable") return [];
      this.item.list.map(i => {
        if (i.type === "select" && i.options.dynamic) {
          arr.push(i.options.dynamicKey);
        }
      });
      return arr;
    },
  ...
  methods: {
    updateInputDefaultValue(name){
      if(this.type === "childTable" && this.dTableDialgoItemIndex.headerName){
        // console.log('要更新的字段',this.formData[this.item.model],this.item.model,this.dTableDialgoItemIndex.scopeIndex,this.dTableDialgoItemIndex.headerName)
        let item = this.formData[this.item.model][this.dTableDialgoItemIndex.scopeIndex];
        this.$set(item,this.dTableDialgoItemIndex.headerName,name)
      }else if (this.type === "inputSearch"){
        this.formData[this.item.model] = name;
      }
    },
    inputSearchFocusHandler(column,scopeIndex,headerIndex){
      if(this.type === "childTable" && column && column.type === 'inputSearch'){
        this.dTableDialgoItemIndex.scopeIndex = scopeIndex;
        this.dTableDialgoItemIndex.headerName = column.model;
        this.dTableDialog = column.options.dTableDialog;
      }else if (this.type === "inputSearch"){
        this.dTableDialog =  this.item.options.dTableDialog;
      }
      this.dTableDialog.visible = true;
    },
    // 子表&&子表中存在下拉框动态数据时候
    getChildTableSelectDictObj() {
      this.childTableSelectDictArr.map(async dicName => {
        let dicList = await this.$store.dispatch(
          "form/fetchDictByKey",
          dicName
        );
        this.$set(this.childTableSelectDictObj, dicName, dicList);
      });
    },
    // (普通表单)下拉框&&动态数据的时候走这里
    async getSelectionDynamicOptions() {
      if (this.item.type !== "select") return [];
      if (!this.item.options.dynamicKey) return [];
      this.selectionDynamicOptions = await this.$store.dispatch(
        "form/fetchDictByKey",
        this.item.options.dynamicKey
      );
      // console.log("vuex中的字典数据", this.selectionDynamicOptions);
    },
    async selectionOptions(item) {
      if (item.type !== "select" || item.options.dynamic) return [];
      let dictName = item.options.dynamicKey;
      let res = await this.getDicts(dictName);
      return this.buildDynamicData(res.data);
    },
    dynamicFun(item) {
      if (item.options.dynamicFun && item.options.eventName) {
        // var function_name=new function(arg1,arg2,...,argN,function_body)
        var val = new Function("item", item.options.dynamicFun);
        console.log("生成的函数---入参", item);
        console.log("生成的函数---函数内容", item.options.dynamicFun);
        // 传参
        val(item);
        console.log(
          "向上找指定名称的组件",
          this.$business.findComponentUpward(this, "WorkDetail")
        );
        // 调用父组件的方法显示弹窗[根据item里面的具体字段来做如下调用父组件函数处理？？？]
        // this.$business.findComponentUpward(this,'WorkDetail').showTableDialogHandler()
        console.log(this.$parent);
        console.log("生成的函数---具体的函数内容", val);
      }
    },
    handleFormButton(type) {
      this.$emit("clickFormButton", type);
    },
    handleAdd(array, model, item) {
      /* if (this.disabled || item.options.disabled) {
        return;
      } */
      // 操作子表添加一行
      let temp = {};
