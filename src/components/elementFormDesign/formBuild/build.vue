  ...
  <div>
    ...
    >
      <el-form
        ...
        :rules="rules"
      >
        <template v-for="(item, i) in formOptions.list">
          <FinalForm
            ...
            "
          />
          <div v-else :key="i" :item="item">
            <slot :name="item.options.customizeName"></slot>
          </div>
        </template>
      </el-form>
    </div>
...
export default {
  ...
  watch: {
    ...
    options: {
      handler(newValue) {
        // console.log('JONS值的变化',JSON.stringify(newValue))
  ...
  methods: {
    ...
    // defaultValue为空对象（即：没有默认值时），为formData添加默认属性
    formDataMap() {
        ...
        array.forEach(item => {
          if (item.model) {
            if (item.type === "number" || item.type === "slider") {
              ...
              // formData[item.model].push(obj) // childTable默认增加一行数据【？？？---这里可以去掉】
              this.childTableColumns.push({
                ...obj,
                model: item.model
              });
              item.options.pageTotal =
                item.type === "childTablePage" ? item.list.length : 0;
              // 如果不是子表单带分页，则设置一个最大的页数
              item.options.pageSize =
                item.type === "childTablePage" ? item.options.pageSize : 100000;
            } else {
              duplicating(duplicate, item);
              formData[item.model] = item.options.defaultValue;
            }
    ...
    rulesMap() {
      ...
      const mapRules = (rules, array) => {
        ...
        array.forEach((child, i) => {
          if (child.model) {
            if (
              child.type === "childTable" ||
              child.type === "childTablePage" ||
              child.type === "customize"
            ) {
              return;
            }
            // 把rule深克隆
