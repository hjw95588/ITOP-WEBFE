<template>
  <div class="contentWrap" v-if="showCont">
    <div class="backWrap">
      <span class="el-icon-arrow-left" @click="closeCont">返回</span>
    </div>
    <div class="infoWrap">
      <div class="contTitle">{{noticCon.noticeTitle}}</div>
      <div class="contTiem">{{noticCon.createTime}}</div>
      <div class="cont" v-html="noticCon.noticeContent"></div>
    </div>
  </div>
</template>
...
<style lang="scss" scoped>
  .contentWrap { position: absolute; top: 0; right: 0; bottom: 0; left: 0; z-index: 99; background: #fff;
    ...
    .infoWrap { height: 65vh; overflow: auto; padding: 10px 20px;
      .contTitle {text-align: center; font-size: 16px; font-weight: 600; line-height: 22px; }
      .contTiem { font-size: 12px; line-height: 18px; color: #999; text-align: right;}
      .cont { width: 95%; margin-left: 10px; color: #999; font-size: 12px; line-height: 18px; text-indent: 10px;
