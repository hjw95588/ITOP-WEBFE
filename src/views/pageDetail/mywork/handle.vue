<template>
  ...
  <div class="app-container">
    <div class="main ebank-form">
      ...
      <div class="content of-auto" :style="{ height: tabContainerHeight }">
        <div
          ...
          v-show="tabsValue === '1'"
        >
          <form-viewer ref="form" :key="workDetail.id"></form-viewer>
        </div>
        <div
          class="tab-container"
          v-if="tabsShow[1]"
    ...
    <el-dialog title="任务" :visible.sync="assignDialog" :close-on-click-modal="false" :append-to-body="true">
...
export default {
  ...
  computed: {
    ...mapGetters(["workDetail", 'routerPath']),
  },
  provide () {
    return {
  ...
  methods: {
    ...
    saveForm() {
      ...
      self.$refs.form.saveForm().catch(() => {
        console.log('我是处理页面的保存')
    ...
    async submitForm() {
      ...
      await this.$refs.form.submitForm().catch(() => {
        console.log('我是处理页面的提交')
        self.submitLoading = false
      });
    },
    setAssign() {
      this.assignForm.taskname = this.workDetail.taskName
      this.transServiceData.taskId = this.workDetail.id
