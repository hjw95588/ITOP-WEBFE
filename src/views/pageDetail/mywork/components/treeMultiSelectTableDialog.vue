...
export default {
  ...
  methods: {
    ...
    handleCloseTagItem(tag) {
      let index = this.selectedUserList.findIndex(
    ...
    /** 表格选中项发生改变 */
    addUserItem(val) {
      let hasUserItem = this.selectedUserList.find(
        item => item.userId === val.userId
      );
      if (!hasUserItem) {
        this.isMultipleChoice ? this.selectedUserList.push(val) : this.selectedUserList.splice(0, 1, val);
      } else {
        //提示已经添加过
        this.$message.error("添加失败，该用户已经添加过！");
      }
    },
    /** 当前页改变 */
    ...
    handleNodeClick(data) {
      ...
      console.log("部门id", this.searchDeptById);
      this.fetchData();
    },
    /** 获取表格数据接口 */
    fetchData() {
