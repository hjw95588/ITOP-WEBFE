...
import { getNodeAssignee, getChartByInst, getFlowNameByInst } from "@/api/work";
...
export default {
  ...
  methods: {
    initPage() {
      // this.flowName = this.workDetail.flowName;
      let processInstanceId = this.processInstanceId
      getChartByInst({ processInstanceId }).then((res) => {
        let data = res.data;
      ...
      // 后端暂无获取flowName接口
      // getFlowNameByInst({processInstanceId}).then(res => {
      //   if (res.code == 200) {
      //     this.flowName = res.msg
      //   }
      // })
    },
    initBpmn() {
