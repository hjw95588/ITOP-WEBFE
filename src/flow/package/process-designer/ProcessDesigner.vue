...
export default {
  ...
  methods: {
    ...
    async createNewDiagram(xml) {
      xml = this.processInfo.resourceName;
      // 将字符串转换成图显示出来
      let xmlString =
        xml ||
        DefaultEmptyXML(
          this.processInfo.flowKey,
          this.processInfo.flowName,
          this.prefix
        );
      try {
    ...
    // 组装表单信息、审批岗位信息
    buildPostData() {
      let postData = deepClone(this.processInfo);
      let flowDTOList = [];
      ...
      nodeList.forEach((i) => {
        if (i.nodeId) {
          ...
          let item = {
            nodeId: i.nodeId,
            formIdStr: formIds.join("|") || "",
            groupId: (i.group && i.group.groupId) || "",
          };
          flowDTOList.push(item);
        }
      });
      postData["flowDTOList"] = flowDTOList;
      delete postData.nodeList;
      return postData;
    },
    // 保存流程图到后台
    async saveProcessAsXml() {
      this.bpmnModeler.saveXML({ format: true }, (err, xml) => {
        if (err) {
          this.$message.error(err);
        } else {
          this.processInfo.resourceName = xml;
          ...
          updateProcess(postData).then((response) => {
            if (response.code === 200) {
              ...
              this.$nextTick(() => {
                this.$business
                  .findComponentUpward(this, "processList")
                  .closeDiagramDialog();
              });
            } else {
