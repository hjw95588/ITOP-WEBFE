  ...
  <div class="process-panel__container" :style="{ width: `${this.width}px` }">
    <el-collapse v-model="activeTab">
      ...
      <el-collapse-item name="extensions" key="extensions">
        ...
        <element-properties ref="extendProperty" :id="elementId" :type="elementType" />
      </el-collapse-item>
...
export default {
  ...
  watch: {
    ...
    elementType(newVal,oldVal){
      console.log('监听元素类型',newVal)
      if(newVal==='UserTask'){
        // 下面的标签上添加属性
        this.$refs.extendProperty.saveAttribute();
        // 这里改变userTask节点的属性
        this.addPropertyForUserTask('activiti:candidateUsers','${approvalIds}')
        this.addPropertyForUserTask('activiti:dueDate','${dueDate}')
        this.addPropertyForUserTask('activiti:assignee','${approvalId}')
        this.addPropertyForUserTask('activiti:candidateGroups','${groupId}')
      }
  ...
  methods: {
    // 给userTask类型的节点添加后端需要的属性
    addPropertyForUserTask(props,val){
      window.bpmnInstances.modeling.updateProperties(this.bpmnElement, {[props]: val});
    },
    initModels() {
