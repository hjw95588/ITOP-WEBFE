<template>
  <div class="panel-tab__content">
      ...
      <el-table :data="ownerListenersList" size="mini" max-height="240" border fit>
        ...
        <el-table-column label="操作" width="90px">
          <template slot-scope="{ row, $index }">
            <el-button size="mini" type="text" @click="openTaskListenerForm(row, $index)">编辑</el-button>
    ...
    <div class="element-drawer__button">
      <el-button size="mini" type="primary" icon="el-icon-plus" @click="openTaskListenerForm(null)">添加监听器</el-button>
    </div>

    <el-drawer :visible.sync="showTaskListenerForm" title="任务监听器/TaskListener" :size="`${this.width}px`" append-to-body destroy-on-close>
...
export default {
  ...
  data() {
    return {
      ...
      listenerFieldForm: {},
      fieldsOfListener: [],
      showTaskListenerForm: false,
      listenerIndex: 0, // 记录打开监听器下标
  ...
  watch: {
    ...
    listenerFieldForm: {
      handler: function (newVal) {
        const objAttr = newVal.fieldType === 'string' ? 'expression' : 'string'
        newVal.hasOwnProperty(objAttr) && delete newVal[objAttr]
      },
      deep: true
    }
  },
  ...
  methods: {
    ...
    // 打开事件监听器侧边栏
    openTaskListenerForm(listener, index) {
      if (listener) {
        if (listener.script) {
          this.listenerForm = {
            ...listener,
      ...
      this.showTaskListenerForm = true;
      this.$nextTick(() => {
    ...
    // 取消编辑
    handleCancel() {
      this.showTaskListenerForm = false;
      this.listenerForm = {};
    ...
    async saveListenerConfig() {
      ...
      const listenerModel = this.moddle.create(`${this.prefix}:TaskListener`, listenerObj);
    ...
    async saveListenerFiled() {
      ...
      if (!validateStatus) return; // 验证不通过直接返回
      const filed = { ...this.listenerFieldForm };
